/**
 * IANA time zones, generated once from Intl.supportedValuesOf('timeZone') and
 * grouped by region for the booking form's <optgroup> list.
 *
 * Generated rather than read at runtime on purpose: the page is statically
 * exported, and a list built during render would be the build machine's ICU
 * data, which can disagree with the browser's and break hydration.
 */
export type TimezoneOption = readonly [value: string, label: string];

export const TIMEZONE_GROUPS: readonly (readonly [region: string, zones: readonly TimezoneOption[]])[] = [
  ["UTC", [["UTC", "UTC"]]],
  [
    "Africa",
    [
...
/** Flat lookup for validating a stored or detected zone against the list. */
export const TIMEZONE_VALUES: ReadonlySet<string> = new Set(
  TIMEZONE_GROUPS.flatMap(([, zones]) => zones.map(([value]) => value))
);
